...
Larmor precession most readily occurs when the nucleus has a net magnetic moment i.e. there is an odd number of nucleons (protons plus neutrons), producing a spin $\half$ system (e.g. $^1H$, $^13C$, $^19F$, etc.). In this case, the magnetic dipole moment $\mu$ of the nucleus is described as
...
In order to directly measure $T_1$ and $T_2$, we must therefore get creative on our measurement techniques. By using specific pulse sequences (see Section IIA,B), the effects of the $\vec{\gamma \Delta B_0}$ can be mitigated and $T_1$ and $T_2$ can be isolated and calculated using Equations 10 and 14, respectively. 

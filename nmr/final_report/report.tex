...
% Larmor precession most readily occurs when the nucleus has a net magnetic moment i.e. there is an odd number of protons, producing a spin $\half$ system.  %% SEONGSHIK ROASTED US FOR THIS
A nucleus has nonzero spin if it contains an odd total number of nucleons (protons plus neutrons). Spin-$\half$ systems (e.g. $^1H$, $^13C$, $^19F$, etc.) arise when there is an odd total nucleon number.  %% fixed based on his feedback
In this case, the magnetic dipole moment $\mu$ of the nucleus is described as

...
def draw_axes(length=13):
    ...
    """
    xaxis = cylinder(
        pos=vector(-length / 2, 0, 0),
        axis=vector(length, 0, 0),
        radius=0.05,
        color=color.gray(0.7),
    )
    xlabel = text(
        pos=-xaxis.pos + vector(0, 0, 0.2),
        text="x'",
        axis=vector(-1, 0, 0),
        align="center",
        up=vector(0, 0, 1),
    ...
    yaxis = cylinder(
        pos=vector(0, -length / 2, 0),
        axis=vector(0, length, 0),
        radius=0.05,
        color=color.gray(0.7),
    )
    ylabel = text(
        pos=-yaxis.pos + vector(0, 0, 0.2),
        text="y'",
        axis=vector(0, 1, 0),
        align="center",
        up=vector(0, 0, 1),
    ...
    zaxis = cylinder(
        pos=vector(0, 0, -length / 2),
        axis=vector(0, 0, length),
        radius=0.05,
        color=color.gray(0.7),
    )
    zlabel = text(
        pos=-zaxis.pos + vector(0, 0, 0.2),
        align="center",
        text="z",
        axis=vector(0, 1, 0),
        up=vector(0, 0, 1),
    ...
    return compound(
        [xaxis, yaxis, zaxis, xlabel, ylabel, zlabel],
        axis=vector(1, 0, 0),
        origin=vector(0, 0, 0),
    )
...
def makeplot(size=510):
    ...
    Mx = gcurve(color=color.red, width=2, visible=show_Mx_checkbox.checked)
    My = gcurve(color=color.green, width=2, visible=show_My_checkbox.checked)
    mz = gcurve(color=color.gray(0.5), width=2, visible=show_Mz_checkbox.checked)
    mr_ave = gcurve(color=color.orange, width=2, visible=show_Mr_ave_checkbox.checked)
...
def spin_initialize(Hwidth=0.04, N=300):
    ...
    if max_field < 1e-6:
        max_field = 1.0  # Don't blow up if max field is zero.
...
def RF_function(phi, RF_type="x-axis"):
    ...
    if RF_type == "x-axis":
        ...
        return sl_h1.value * omega0 * (1 + cos(2.0 * phi)), sl_h1.value * omega0 * sin(
            2.0 * phi
        )
    elif RF_type == "y-axis":
...
Spin_number = menu(
    choices=["1", "5", "10", "100", "300", "1000", "3000"],
    selected="3000",
    pos=scene.caption_anchor,
    bind=Spin_select,
)
scene.append_to_caption(" <i>T</i><sub>2</sub><sup>*</sup>: ")
fwhm_w = winput(width=50, text="1.0", pos=scene.caption_anchor, bind=FWHM_select)
...
scene.append_to_caption(" ")
frame_button = button(
    text="<b>Rotating</b>/Lab", pos=scene.caption_anchor, bind=Rotate_frame
)

# Row 3 -----------------------------------------
scene.append_to_caption("\n  Simulation speed:")
sl_sim_speed = slider(
    min=4, max=200, value=100, step=2, top=10, length=150, right=15, bind=Set_sim_speed
)
wt_sim_speed = wtext(text="{:.0f}%".format(sl_sim_speed.value))
scene.append_to_caption("  ")
RF_frame_checkbox = checkbox(
    text="RF frame only",
    pos=scene.caption_anchor,
    checked=False,
    bind=Rotating_frame_select,
)
...
RF_menu = menu(
    choices=["x-axis", "y-axis", "oscillating"],
    pos=scene.caption_anchor,
    bind=RF_select,
)
...
scene.append_to_caption("  ")
Instant_checkbox = checkbox(
    text="'Instant' RF", pos=scene.caption_anchor, checked=True, bind=Instant_select
)
scene.append_to_caption("  N<sub>B</sub>: ")
NumB_pulse_menu = menu(
    choices=["0", "1", "2", "3", "5", "8", "10", "15", "20"],
    selected="0",
    pos=scene.caption_anchor,
...
# Row 6 -----------------------------------------
scene.append_to_caption("\n ")
Apulse_checkbox = checkbox(
    text="A pulse: ", pos=scene.caption_anchor, checked=True, bind=Apulse_on
)
Apulse_menu = menu(
    choices=["pi/2", "pi", "pi+error", "optimal"],
    selected="pi/2",
    pos=scene.caption_anchor,
    bind=Apulse_select,
)
...
# Row 7 ----------------------------------------
scene.append_to_caption("\n ")
Bpulse_checkbox = checkbox(
    text="B pulse: ", pos=scene.caption_anchor, checked=False, bind=Bpulse_on
)
Bpulse_menu = menu(
    choices=["pi/2", "pi", "pi+error", "optimal"],
    selected="pi",
...
w_delta = winput(width=30, text="20", pos=scene.caption_anchor, bind=delta_select)
...
sl_h1 = slider(min=0.01, max=1.2, value=0.4, length=250, bind=Set_h1, left=25, right=15)
...
scene.append_to_caption("  ")
show_signal_checkbox = checkbox(
    bind=Show_signal,
    text='<font color="blue"><i>M</i><sub>&perp;</sub></font>  ',
    checked=True,
)
show_Mz_checkbox = checkbox(
    bind=Show_Mz, text='<font color="gray"><i>M<sub>z</sub></i></font>  ', checked=False
)
show_Mr_ave_checkbox = checkbox(
    bind=Show_Mr_ave,
    text='<font color="orange">&langle;<i>M<sub>&perp;</sub></i>&rangle;</font>  ',
    checked=False,
)
show_Mx_checkbox = checkbox(
    bind=Show_Mx, text='<font color="red"><i>M<sub>x</sub></i></font>  ', checked=False
)
show_My_checkbox = checkbox(
    bind=Show_My, text='<font color="green"><i>M<sub>y</sub></i></font>', checked=False
)
scene.append_to_caption("  \n\n")
...
wt_time = label(
    text="Time: {:.1f}".format(t), pixel_pos=True, align="right", pos=vector(160, 60, 0)
)
wt_next_pulse = label(
    text=" ", pos=vector(180, 60, 0), align="left", pixel_pos=True, box=False, opacity=0
)
...
M = arrow(pos=vector(0, 0, 0), axis=m0, shaftwidth=0.3, round=True, color=color.blue)
B = arrow(
    pos=vector(0, 0, 0), axis=Om0 / gamma, shaftwidth=0.3, round=True, color=color.green
)
...
def calc_OMEGA():
    ...
    this_OMEGA = vector(0.0, 0.0, 0.0)  # Local omega
    ...
    if Npulse > 0:
        ...
        if sequence_state == "Fire_A":
            ...
            if Npulse > 0:  # Set time to next pulse, if it exists
                ...
                wt_next_pulse.text = "B pulse {:d} at {:.1f}".format(
                    N_start - Npulse, t_nextPulse
                )
    ...
    if RF_on or (t < pulse_time):
        ...
        instant = (
            instant_pulse  # instant prevents spins from evolving under local field
        )
